...
DEVTOOLSET_PATH=/opt/rh/devtoolset-8/root
...
pushd
cd openssl-1.1.0h
./config --prefix=${DEVTOOLSET_PATH}/usr --openssldir=${DEVTOOLSET_PATH}/usr/ssl no-shared threads ${EXTRA_CXXFLAGS} ${EXTRA_LDFLAGS}
make && make install
popd
...
cd cmake-3.11.4
. /etc/profile.d/devtoolset-8.sh
CXXFLAGS=${EXTRA_PIC_CXXFLAGS} CFLAGS=$CXXFLAGS CPPFLAGS=$CXXFLAGS LDFLAGS="${EXTRA_LDFLAGS} -pthread" ./configure --prefix=${DEVTOOLSET_PATH}/usr
make && make install
popd
